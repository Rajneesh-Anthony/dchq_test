class SaleDecorator < Draper::Decorator
  ...
  def customer_sale_products
    model.sale_products.map do |p|
      ...
      product_name = "#{ p.quantity } x #{ p.sale_productable.try(:name) } (#{(h.formatted_currency(p.unit_price))})"
      if p.sale_productable_type == 'Product'
        ...
        h.link_to product_name, h.event_path(p.sale_productable.try(:event))
      else
        product_name
      end
    end * ', '
  ...
  def tax_rate_total_abs
    h.formatted_currency(model.tax_rate_total ? model.tax_rate_total.abs : 0)
  end
