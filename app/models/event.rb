...
#  additional_equipment   :string(255)
#  price                  :decimal(11, 2)
...
#  parent_id              :integer
#  notes                  :text
#  enable_booking         :boolean          default(TRUE), not null
#  limit_of_registrations :integer
#  location               :string(255)
#  instructions           :text
#  cancel                 :boolean          default(FALSE)
#  boat_id                :integer
#  number_of_frequencies  :integer          default(0)
...
class Event < ActiveRecord::Base
  ...
  include PgSearch
  filterrific(  
  default_filter_params: { starts_at: 'starts_at_desc' },
  available_filters: [
    :starts_at,
    :search_query,
    :with_started_at_gte
  ...
  attr_accessible :name, :event_type_id, :certification_level_id, :event_trip_id, :starts_at, :ends_at,
                  ...
                  :event_user_participants_attributes, :event_description_id, :image, :deleted_at, :all_day
  ...
  scope :public_events,      ->{ where(private: false) } #todo: fix as this is inverted for public calendar
  scope :visible,      ->{ where(private: true) } #todo:  fix as this is inverted for public calendar
  scope :private_events,->{ where(private: false) } #todo: fix as this is inverted for private calendar
