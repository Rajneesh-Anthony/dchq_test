...
#   after_create :approve
  ...
  def approve
    Rails.logger.info('[EventCustomerParticipant] call #approve')
  ...
  def send_bookings_not_paid_emails
    Rails.logger.info("[EventCustomerParticipant] call #send_bookings_not_paid_emails; self: #{self}")
  ...
  def send_notification_email
    # event.users.where("email IS NOT NULL").each do |user|
    #   SaleMailer.delay.send_update_ecp_email( self, user.email )
    # end

    #event.customers.where("email IS NOT NULL").each do |customer|
