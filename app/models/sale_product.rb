class SaleProduct < ActiveRecord::Base
  ...
  def tax_rate_amount
    return 0 if sale.customers.first.try(:zero_tax_rate?) || sale_productable.nil?
    tax = if sale_productable_type == 'Product' && sale.complete? && !tax_rate.nil?
            tax_rate
          elsif sale_productable.present? && sale_productable.class.base_class == EventCustomerParticipantOptions::EventCustomerParticipantOption
            sale_productable.send(:method).tax_rate.amount
          else
            sale_productable.try(:tax_rate_amount)
          end
